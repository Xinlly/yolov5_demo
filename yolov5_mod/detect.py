...
def run(
    ...
    show_temper=False,  # show temperature
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_txt:  # Write to file
                        ...
                        # xywh = (xyxy2xywh(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()  # pixel xywh
                        line = (
                            (cls, *xywh, conf) if save_conf else (cls, *xywh)
                        )  # label format
                        with open(f"{txt_path}.txt", "a") as f:
                            f.write(
                                f"{label}" + (" %g " * len(line)).rstrip() % line + "\n"
                            )
...
def parse_opt():
    ...
    parser.add_argument(
        "--show-temper", action="store_true", help="show temperature in label"
    )
    parser.add_argument(
